...
public class PolyTest{
    ...
    public void Test6add(){
        //Adding onto an empty polynomial
        Poly p1 = new Poly();
        Poly p2 = new Poly(3,2);// 3x^2
        Poly p3 = p1.add(p2);//3x^2
        assertEquals(p3,p2);

        //testing if polynomials with negative coefficients cancel out polynomials with positive coefficients of the same magnitude
        //creating a cubic monomial and its additive inverse.
        Poly p4 = new Poly(1,3);//x^3
        Poly p5 = new Poly(-1,3);//-x^3
        Poly p6 = p4.add(p5);//0
        Poly p7 = new Poly();//0
        //adding them together should return an empty polynomial
        assertEquals(p6,p7);
        
        //creating a quadratic monomial and its additive inverse.
        Poly p8 = new Poly(4,2);//4x^2
        Poly p9 = new Poly(-4,2);//-4x^2
        Poly p10 = p8.add(p9);//0
        //adding these quadratic monomials to the cubic monomials
        Poly p11 = p4.add(p8);//x^3 + 4x^2
        Poly p12 = p5.add(p9);//-x^3 -4x^2
        Poly p13 = p11.add(p12);//0
        //checking that the sum of the individual monomials is equal to 0 as well as the sum of the monomials when added with their respective cubic monomials
        assertEquals(p13,p7);
        assertEquals(p10,p13);
        assertEquals(p10,p7);
        
        //creating a linear monomial and its additive inverse
        Poly p14 = new Poly(8,1);//8x
        Poly p15 = new Poly(-8,1);//-8x
        Poly p16 = p14.add(p15);//0
        //adding the linear monomials to their respective polynomials
        Poly p17 = p11.add(p14);//x^3 + 4x^2 +8x
        Poly p18 = p12.add(p15);//-x^3 -4x^2 -8x
        Poly p19 = p17.add(p18);//0
        //checking that the sum of the individual monomials is equal to 0 as well as the sum of the monomials when added with their respective polynomials
        assertEquals(p19,p7);
        assertEquals(p19,p13);
        assertEquals(p16,p7);
        assertEquals(p16,p19);
        assertEquals(p19,p10);
        
        //creating a constant and its additive inverse
        Poly p20 = new Poly(6,0);//6
        Poly p21 = new Poly(-6,0);//6
        Poly p22 = p20.add(p21);//0
        //adding the constants to their respective polynomials
        Poly p23 = p17.add(p20);//x^3 + 4x^2 +8x + 6
        Poly p24 = p18.add(p21);//-x^3 -4x^2 -8x -6
        Poly p25 = p23.add(p24);//0
        //checking that the sum of the individual monomials is equal to 0 as well as the sum of the monomials when added with their respective polynomials
        assertEquals(p22,p7);
        assertEquals(p19,p22);
        assertEquals(p25,p7);
        assertEquals(p16,p25);
        assertEquals(p22,p25);
        
    }

    @Test
    public void Test7subtractmulti(){
        //checking if subtracting a term from zero is the same as multiplying it by -1
        Poly p12 = new Poly(); //0
        Poly p13 = new Poly(-1,0); //-1
        Poly p14 = new Poly(1,2);//x^2
        Poly p15 = p12.subtract(p14);//-x^2
        Poly p16 = p13.multiply(p14);//-x^2
        assertEquals(p15,p16);
        
        //checking if subtracting a term from its additive inverse is the same as multiplying it by 2
        Poly p1 = new Poly(3,2);// 3x^2
        Poly p2 = new Poly(-3,2);// -3x^2
        Poly p3 = p1.subtract(p2);//6x^2
        Poly p4 = new Poly(2,0);//2
        Poly p5 = p1.multiply(p4);//6x^2
        assertEquals(p3,p5);
        
        Poly p6 = new Poly(-3,3);//-3x^3
        Poly p7 = new Poly(3,3);//3x^3
        Poly p8 = p1.subtract(p6);//3x^3 + 3x^2
        Poly p9 = p2.subtract(p7);//-3x^3-3x^2
        Poly p10 = p8.subtract(p9);//6x^3 + 6x^2
        Poly p11 = p4.multiply(p8);//6x^3 + 6x^2
        assertEquals(p10,p11);
        
        Poly p17 = new Poly(-4,4);//-4x^4
        Poly p18 = new Poly(4,4);//4x^4
        Poly p19 = p8.subtract(p17); //4x^4 + 3x^3 + 3x^2
        Poly p20 = p9.subtract(p18); //-4x^4 -3x^3 -3x^2
        Poly p21 = p19.subtract(p20); //8x^4 + 6x^3 + 6x^2
        Poly p22 = p4.multiply(p19); //8x^4 + 6x^3 + 6x^2
        assertEquals(p21,p22);
    }
    
    @Test
    public void Test8divide(){
        //This test is to check what dividing by a constant i.e a polynomialw whose greatest exponent is 0 would return
        Poly p1 = new Poly(2,0);//2
        Poly p2 = new Poly(2,3);//2x^3
        Poly p3 = new Poly(1,3);//x^3
        Poly p4 = p2.divide(p1);//x^3
        assertEquals(p3,p4);
        
        //checks if it works for a fraction constant
        Poly p5 = new Poly(4,2);//4x^2
        Poly p6 = new Poly(8,2);//8x^2
        Poly p7 = new Poly(0.5,0);//half
        Poly p9 = p3.add(p5);//x^3 + 4x^2
        Poly p10 = p2.add(p6);//2x^3 + 8x^2
        Poly p8 = p5.divide(p7);//8x^2
        Poly p11 = p9.divide(p7);//2x^3 + 8x^2
        assertEquals(p6,p8);
        assertEquals(p11,p10);
        
        // this checks for if it works for a negative constant
        Poly p12 = new Poly(-3,0);//-3
        Poly p13 = new Poly(6,1);//6x
        Poly p14 = new Poly(-6,2);//-6x^2
        Poly p15 = p13.add(p14);//6x - 6x^2
        Poly p19 = new Poly(-2,1);//-2x
        Poly p20 = new Poly(2,2);//2x^2
        Poly p21 = p19.add(p20);
        Poly p22 = p15.divide(p12);
        assertEquals(p21,p22);
        //this checks for divison by a polynomial with a negative exponent
        Poly p16 = new Poly(-1,0);//-1
        Poly p18 = new Poly(1,-1);// 1/x
        Poly p17 = p12.divide(p18).divide(p7).divide(p16);//6x
        assertEquals(p13,p17);
        
    }
    
    @Test
    public void Test9multi(){
        //check if it sums out monomials with common exponents after evaluation
        Poly p1 = new Poly(1,3);//x^3
        Poly p2 = new Poly(1,2);//x^2
        Poly p3 = new Poly(1,1);//x
        Poly p4 = p1.add(p2);//x^3 + x^2
        Poly p5 = p2.add(p3);//x^2 + x
        Poly p6 = p4.multiply(p5);
        Poly p8 = new Poly(2,0);//2
        Poly p7 = p8.multiply(p2).multiply(p2);//2x^4
        Poly p9 = new Poly(1,5);//x^5
        Poly p10 = p9.add(p1).add(p7);
        assertEquals(p10,p6);
        
        //checks if the exponents in a polynomial reduce after being multiplied 
        //by another with a negative exponent
        Poly p11 = p10.subtract(p7);//x^5 + x^3
        Poly p12 = new Poly(1,-2);// 1/x^2
        Poly p13 = p11.multiply(p12);//x^3 + x^2
        assertEquals(p13,p4);
    }
    
    @Test
    public void Test10allOpps(){
        //This test is to show that precedence matters in evaluating polynomial 
        //arithmetic as it does in regular arithmetic
        Poly p1 = new Poly(1,3);//x^3
        Poly p5 = new Poly(4,2);//4x^2
        Poly p3 = new Poly(6,0);//6
        Poly p2 = new Poly(1,2);//x^2
        Poly p18 = new Poly(4,4);//4x^4
        
        Poly p10 = p1.add(p5);//x^3 + 4x^2
        Poly p11 = p10.subtract(p2);//x^3 + 3x^2
        Poly p12 = p11.divide(p3).multiply(p18);
        Poly p14 = p1.subtract(p2);
        Poly p15 = p14.divide(p3);
        Poly p16 = p15.multiply(p18).add(p5);
        assertNotEquals(p10,p15);
        
        //SHowing that adding and subtracting a polynomial from a given number is the
        //same as multiplying and dividing a polynomial from a given polynomial
        Poly m = p1.add(p5).add(p3).add(p2).add(p18);
        Poly n = m.add(m).subtract(m);
        assertEquals(m,n);
        Poly p = m;
        Poly q = p.multiply(p).divide(m);
        assertEquals(p,q);
        assertEquals(p,m);
        assertEquals(n,p);
    }
